        ...
        uint16_t length, void FAR *context)
{
	...
	if (strncmp(frame->payload, "Join confirmed", payload_length) == 0)
	    	rx_ready = TRUE;
	else if (strncmp(frame->payload, "0", payload_length) == 0)
	{
		printf("\nCLOSED");
		receivdcmd = 0;
		rx_ready = TRUE;
	}
	else if (strncmp(frame->payload, "1", payload_length) == 0)
	{
		printf("\nOPEN");
		receivdcmd = 1;
		rx_ready = TRUE;
...
void main(void)
{
	...
	// Initialize the GPIO7 pin to output
	ret = gpio_config(XPIN_7, GPIO_CFG_OUTPUT);
	if (ret != 0)
		printf("error");
	...
	for (;;) {
		...
		//printf("\nenv payload %s", env.payload);


		if (receivdcmd != 0 && receivdcmd != 1)
			...
			if ( receivdcmd == 0 )
			{   
				printf("CLOSED in main loop");
				gpio_set(XPIN_7,0);
				if (gpio_get(XPIN_7))
					sprintf(s, "c1:1000.00 ERROR");	
				else
					sprintf(s, "c1:1.00 CLOSED");
				xbee_transparent_serial_str(&env, s);
			}
			else
			{
				printf("OPEN in main loop");
				gpio_set(XPIN_7,1);
				if (gpio_get(XPIN_7))
					sprintf(s, "c1:1.00 OPEN");
				else
					sprintf(s, "c1:1000.00 ERROR");	
				xbee_transparent_serial_str(&env, s);
			}
